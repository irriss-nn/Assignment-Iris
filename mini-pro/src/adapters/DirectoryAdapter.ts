...
export function useFileAdapter(metadata: Pick<Directory.FileContent, 'id'>) {
  ...
  const copyFile = useMemo(() => (newParentId: Directory.FileMetadata['parentId']) => {
    File.saveFileMetadata({
      ...fileMetadata,
...
export function useFolderAdapter(metadata: Pick<Directory.FolderMetadata, 'id' | 'parentId'> = Directory.RootNode) {
  ...
  const createFile = useMemo(() => (params: { name: string, content?: string }) => {
    // Destructure content from params, with a default value if not provided
    const { name, content = '' } = params
  
    return File.createFile({
      parentId: metadata.id,
      name: name,
      content: content, // Assuming your createFile function can handle a content argument
    }, localDirectoryDatabase, directoryState)
  }, [metadata.id])
